...
export function ProjectHeader({ title, technologies, description, image }: ProjectHeaderProps) {
  return (
    <header className="w-full max-w-5xl mx-auto px-4 py-8 md:py-12">
      <div className="space-y-6">
        <Link 
          href="/projects"
          className="inline-flex items-center text-sm font-medium text-muted-foreground hover:text-ink-strong transition-colors mb-4"
        >
        ...
        <div className="flex gap-2 flex-wrap">
          {technologies.filter(tech => tech && tech.trim() !== "").map((tech) => (
            <Badge key={tech} variant="secondary" chip="tag-sm">
              {tech}
            </Badge>
          ))}
        </div>
        <h1 className="text-4xl md:text-6xl font-bold tracking-tight text-foreground leading-tight max-w-4xl">
          {title}
        </h1>
