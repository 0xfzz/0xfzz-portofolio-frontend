...
export function MarkdownRenderer({
  ...
  return (
    <ReactMarkdown
      ...
      components={{
        h1: ({ children }) => (
          <h1 className="text-[32px] font-extrabold text-[#1a1a1a] tracking-tight leading-tight mt-10 mb-6 first:mt-0">
            {children}
          </h1>
        ),
        h2: ({ children }) => (
          <h2 className="text-[20px] font-bold text-[#1a1a1a] tracking-tight mt-8 mb-4 border-t border-[#eee] pt-8 first:mt-0 first:pt-0 first:border-0">
            {children}
          </h2>
        ),
        h3: ({ children }) => (
          <h3 className="text-[14px] font-extrabold text-[#1a1a1a] uppercase tracking-wider mt-6 mb-3 border-t border-[#eee] pt-6 first:mt-0 first:pt-0 first:border-0">
            {children}
          </h3>
        ),
        p: ({ children }) => {
          return (
            <div className="text-[18px] text-[#444] leading-relaxed font-sans font-medium content-area-p tracking-tight opacity-90 mb-5 last:mb-0">
              {children}
            </div>
          );
        },
        li: ({ children }) => (
          <li className="flex items-start gap-4 group mb-4 last:mb-0">
            ...
            <span className="text-[14px] text-[#555] font-medium leading-tight group-hover:text-[#1a1a1a] transition-colors">
              {children}
            </span>
          </li>
        ),
        code: ({ node, inline, className, children, ...props }: any) => {
          ...
          if (!inline && content.includes("\n")) {
            return (
              <div className="my-8 rounded-md overflow-hidden border border-[#f0f0f0] bg-[#1e1e1e] shadow-xl">
          ...
          return (
            <code className="px-1.5 py-0.5 rounded-sm text-[13px] font-mono text-[#1a1a1a] bg-[#f5f5f5] border border-[#ebebeb] mx-0.5">
              {children}
            </code>
          );
        },
        ...
        table: ({ children }) => (
          <div className="my-10 overflow-x-auto border border-[#f0f0f0] rounded-sm bg-white shadow-sm">
